...
(function () {
  ...
  function renderEpisodes() {
    ...
    state.episodes.forEach(function (episode, index) {
  ...
  function loadFallbackEpisodes() {
    ...
    renderEpisodes();
    updateStatus("Live feed is temporarily unavailable. Loaded fallback Fabric playlist.");
  }
  ...
  function currentEpisodeText() {
    ...
    const snippet = episode.description ? episode.description.slice(0, 280) : "Open the post for details.";
    return "Fabric Updates podcast. Episode " + (state.currentIndex + 1) + ". " + episode.title + ". " + snippet;
  }
  ...
  function speakCurrentEpisode() {
    ...
    utterance.onstart = function () {
      ...
      state.isPaused = false;
      const title = state.episodes[state.currentIndex] ? state.episodes[state.currentIndex].title : "";
  ...
  async function initPodcast() {
    ...
    updateStatus("Loading latest posts...");
    state.isLoading = true;

    try {
      const feedText = await fetchFeedText();
      const parsedEpisodes = parseEpisodes(feedText);
